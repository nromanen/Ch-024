define('CalendarView', ['jquery', 'underscore', 'backbone', 'moment', 'jqueryui', 'fullcalendar', 'EventModel', 'ownpopover', 'EventView'],
    function($, _, Backbone, moment, jqueryui, fullcalendar, EventModel, ownpopover, EventView) {
    ...
    var CalendarView = Backbone.View.extend({
        ...
        _convertHexColorToRGB: function(color) {
            return "rgba(" + parseInt(color.substring(1, 3), 16) + "," + parseInt(color.substring(3, 5), 16) + "," + parseInt(color.substring(5, 7), 16) + ", .5)";
        },
        ...
        _showEventModal: function(eventObject) {
            var eventModel = this.eventsCollection.findWhere({
